...
require_once(STYLESHEETPATH . '/inc/texts/texts.php');
...
function afdm_setup() {
	...
	add_image_size('page-featured', 696, 300);
	add_image_size('featured-rectangle', 1352, 400, true);
	load_textdomain('arteforadomuseu', get_stylesheet_directory().'/languages/pt_BR.mo');
}
...
function afdm_scripts() {
	...
	wp_enqueue_script('afdm', get_stylesheet_directory_uri(). '/js/arteforadomuseu.js', array('responsive-nav', 'shadowbox', 'jquery-effects-slide'), '0.1.9');
}
...
function afdm_register_lib() {
	...
	wp_enqueue_script('shadowbox', get_stylesheet_directory_uri() . '/lib/shadowbox/shadowbox.js', array('jquery'), '3.0.3');
	wp_enqueue_style('shadowbox', get_stylesheet_directory_uri() . '/lib/shadowbox/shadowbox.css');

	wp_register_script('bxslider', get_stylesheet_directory_uri() . '/lib/jquery.bxslider/jquery.bxslider.min.js');
	wp_enqueue_script('bxslider');
	wp_enqueue_style('bxslider', get_stylesheet_directory_uri() . '/lib/jquery.bxslider/jquery.bxslider.css');

	// wp_register_style('font-droidserif', 'http://fonts.googleapis.com/css?family=Droid+Serif:400,700,400italic,700italic');
...
register_sidebar( array(
    'name'         => __( 'Yellow square' ),
    'id'           => 'yellow-square',
    'description'  => __( 'Only on home' ),
    'before_title' => '',
    'after_title'  => '',
) );

register_sidebar( array(
    'name'         => __( 'Yellow square footer' ),
    'id'           => 'yellow-square-footer',
    'description'  => __( 'Yellow square on footer' ),
    'before_title' => '',
    'after_title'  => '',
) );


function get_video($content, $type) {
	if(preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $content, $url)) {
		$youtube_id = $url[1];
		$video_thumb_url = 'http://img.youtube.com/vi/' . $youtube_id . '/0.jpg';
        ...
        $regexstr = '~
            ...
            ~ix';
		preg_match($regexstr, $content, $url);
		$vimeo_id = $url[1];
		$vimeo_xml = "http://vimeo.com/api/v2/video/" . $vimeo_id . ".php";
		$vimeo_hash = unserialize(file_get_contents($vimeo_xml));
		$video_thumb_url = $vimeo_hash[0]['thumbnail_medium'];
...
function category_color($cat_id, $type = "bg") {
	$category_color = get_term_field('description', $cat_id, 'category');;
	$colors = array(
		"vermelho",
		"azul",
		"verde"
	);
	if (!in_array($category_color, $colors)) {
		$category_color = "default";
	}
	return $type."-".$category_color;
}